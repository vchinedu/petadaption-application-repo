    ...
    stages {
        ...
        stage('Deploy to stage') {
            steps {
                sshagent(['ansible-key']) {
                    sh 'ssh -t -t ec2-user@10.0.2.150 -o strictHostKeyChecking=no "ansible-playbook -i /etc/ansible/stage-hosts /etc/ansible/stage-playbook.yml"'
                }
            }
        }
        stage('check stage website availability') {
            steps {
                ...
                script {
                    def response = sh(script: "curl -s -o /dev/null -w \"%{http_code}\" https://stage.egoigweimpexlinkinternational.com", returnStdout: true).trim()
                    if (response == "200") {
        ...
        stage('Deploy to prod') {
            steps {
                sshagent(['ansible-key']) {
                    sh 'ssh -t -t ec2-user@10.0.2.150 -o strictHostKeyChecking=no "ansible-playbook -i /etc/ansible/prod-hosts /etc/ansible/prod-playbook.yml"'
                }
            }
        }
        stage('check prod website availability') {
            steps {
                 sh "sleep 90"
                 sh "curl -s -o /dev/null -w \"%{http_code}\" https://prod.egoigweimpexlinkinternational.com"
                script {
